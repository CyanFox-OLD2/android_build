...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cf_") ; then
       CF_BUILD=$(echo -n $1 | sed -e 's/^cf_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $CF_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CF_BUILD=
    fi
    export CF_BUILD
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var CF_VERSION)
        ZIPFILE=CyanFox-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cyanfox.device=$CF_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CF_BUILD, run away!"
    fi
}
...
function installboot()
{
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.cm.device=$CF_BUILD");
    then
